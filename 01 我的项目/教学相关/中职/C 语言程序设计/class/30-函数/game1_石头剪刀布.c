 ...
 *
 * 1. 基础练习：
 *    - 让学生添加胜负次数统计功能（增加winCount、loseCount、drawCount变量）
 *    - 在游戏结束时显示总战绩
 *
 * 2. 进阶练习：
 *    - 改用 switch-case 语句重写 judgeWinner 函数
 *    - 增加"最多玩N局"的限制（例如：最多玩5局）
 *
 * 3. 创意拓展：
 *    - 增加"石头剪刀布蜥蜴史波克"（5种选择）
 *    - 添加难度选择：简单模式（电脑随机）、困难模式（电脑总是出克制玩家上一次的拳）
 *
 * 【常见学生错误】
 *
 * 1. 忘记 srand(time(NULL))，导致每次运行电脑出拳都一样
 * 2. scanf("%c") 前忘记加空格，导致读取到上一次的回车符
 * 3. judgeWinner 函数的逻辑判断错误（需要画真值表帮助理解）
 * 4. rand() % 3 生成的是 0-2，需要 +1 才是 1-3
 */
