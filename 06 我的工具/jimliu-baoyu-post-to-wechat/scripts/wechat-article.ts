...
async function selectAndReplacePlaceholder(session: ChromeSession, placeholder: string): Promise<boolean> {
  const result = await session.cdp.send<{ result: { value: boolean } }>('Runtime.evaluate', {
    expression: `
      (function() {
        const editor = document.querySelector('.ProseMirror') || document.querySelector('[contenteditable="true"]') || document.querySelector('.edui-editor-body');
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  try {
    ...
    // Try multiple selectors for editor content area (WeChat may update their editor)
    const editorSelectors = ['.ProseMirror', '[contenteditable="true"]', '.edui-editor-body', '#ueditor_0'];
    let editorClicked = false;
    for (let retry = 0; retry < 3 && !editorClicked; retry++) {
      if (retry > 0) {
        console.log(`[wechat] Retrying editor detection (attempt ${retry + 1})...`);
        await sleep(3000);
      }
      for (const sel of editorSelectors) {
        try {
          await clickElement(session, sel);
          console.log(`[wechat] Editor found with selector: ${sel}`);
          editorClicked = true;
          break;
        } catch {}
      }
    }
    if (!editorClicked) throw new Error('Editor not found with any known selector');
    await sleep(500);
